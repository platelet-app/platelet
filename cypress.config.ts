...
// GraphQL errors come back in a 200 response, so executeIamGraphqlRequest
// never rejects on them (specs rely on inspecting result.errors). Cleanup
// code uses this wrapper instead so that failures are loud rather than
// silently leaving test tenants and users behind.
