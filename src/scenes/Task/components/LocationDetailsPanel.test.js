...
describe("LocationDetailsPanel", () => {
    ...
    `(
        ...
        async ({ locationKey }) => {
            ...
            expect(saveSpy).toHaveBeenCalledWith({
                ...fakeTaskModel,
                id: expect.any(String),
                [`${locationKey}Id`]: expect.any(String),
            });
        }
    );
    ...
    `("edit the contact information of a location", async ({ locationKey }) => {
        const fakeInputData = "new data";
        const fakeModel = await DataStore.save(
            new models.Location({
                ...mockLocations[1],
                listed: 0,
            })
    ...
    `(
        ...
        async ({ locationKey }) => {
            ...
            userEvent.click(await screen.findByText("new preset"));
            const label =
            ...
            await waitFor(() => {
                expect(saveSpy).toHaveBeenCalledWith({
                    ...task,
                    [`${locationKey}Id`]: mockPreset2.id,
                });
            });
        }
    );
    test("error on task get", async () => {
        const querySpy = jest
