...
import {
    ...
    unsetTaskPickupDestinationRequest, updatePickupLocationAndUpdateTaskRequest
...
function PickUpDetails(props) {
    ...
    const pickupPostingSelector = createPostingSelector([updateTaskPickupTimePrefix]);
    ...
    function onSelectPickupFromSaved(location) {
        ...
        if (locationUUID) {
            dispatch(setTaskPickupDestinationRequest(props.taskUUID, locationUUID))
        }
    }
    ...
    function onChangePickupLocation(value, makeNew = false) {
        if (props.location) {
            if (makeNew) {
                dispatch(addNewPickupLocationAndSetTaskRequest(props.taskUUID, {address: value}))
            } else {
                dispatch(updatePickupLocationAndUpdateTaskRequest(props.taskUUID, {address: value}))
            }
        } else {
            dispatch(addNewPickupLocationAndSetTaskRequest(props.taskUUID, {address: value}))
        }
    ...
    function onChangeTimePickedUp(value) {
        if (value || value === null)
            dispatch(updateTaskPickupTimeRequest(props.taskUUID, {time_picked_up: value}))
    }
