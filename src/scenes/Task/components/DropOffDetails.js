...
import {updateTaskDropoffTimePrefix, updateTaskDropoffTimeRequest} from "../../../redux/tasks/TasksActions";
import {
    addNewDropoffLocationAndSetTaskRequest, setTaskDropoffDestinationRequest, unsetTaskDropoffDestinationRequest,
...
function DropOffDetails(props) {
    ...
    const dropoffPostingSelector = createPostingSelector([updateTaskDropoffTimePrefix]);
    ...
    function onChangeTimeDropoff(value) {
        if (value || value === null)
            dispatch(updateTaskDropoffTimeRequest(props.taskUUID, {time_dropped_off: value}))
    }

    function onChangeDropoffLocation(value, makeNew = false) {
        if (props.location) {
            if (makeNew) {
                dispatch(addNewDropoffLocationAndSetTaskRequest(props.taskUUID, {address: value}))
            } else {
                dispatch(updateDropoffLocationAndUpdateTaskRequest(props.taskUUID, {address: value}))
            }
        } else {
            dispatch(addNewDropoffLocationAndSetTaskRequest(props.taskUUID, {address: value}))
        }
    ...
    function onSelectDropoffFromSaved(location) {
        ...
        if (locationUUID) {
            dispatch(setTaskDropoffDestinationRequest(props.taskUUID, locationUUID))
        }
    }
    return (
                ...
                <Grid item>
                    <LabelItemPair label={"Time delivered"}>
                        <TimePicker
                            ...
                            disabled={isPostingDropoffTime || props.disableTimeButton}
