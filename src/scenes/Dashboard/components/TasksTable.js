...
function tasksDataColumns(tasks, hideDelivered) {
    ...
    const concatTasks = Object.entries(tasks).reduce(
        (accumulator, [key, value]) => {
            if (hideDelivered && key === "tasksDelivered") return accumulator;
    ...
    const columns = [
        {
            ...
            customSort: (a, b) =>
                a.colourCode.index < b.colourCode.index ? -1 : 1,
        },
        ...
        { title: "Assignees", field: "assignees" },
        {
            ...
            field: "time_of_call",
            render: (rowData) => (
                <Moment local calendar style={{ fontSize: "14px" }}>
                    {rowData.time_of_call ? rowData.time_of_call : ""}
                </Moment>
            ),
            defaultSort: "desc",
        },
    ...
    const actions = [
        {
            ...
            onClick: (event, rowData) => {
                history.push(`/task/${encodeUUID(rowData.uuid)}`);
            },
        },
        {
        ...
        >
            {Object.entries(orderTaskList(props.tasks)).map((taskList) => {
                if (
                    ...
                    props.excludeColumnList.includes(taskList[0])
                )
                    return <></>;
                let newTaskButton = "";
