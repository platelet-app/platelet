...
// reducer just to keep the dashboard state up to date from task dialog
export function onChangeTask(state = {}, action) {
    switch (action.type) {
        case ON_CHANGE_TASK:
            return action.data;
        default:
...
export function tasks(state = initialTasksState, action) {
    ...
    if (categoriesCheck.includes(action.type)) {
        ...
        if (parent.taskGroup) {
            ...
            filteredTasks = removeParentFromTasks(
                state.tasks,
                parent.listType,
                task.parent_id
            );
            if (
                [
                    taskCategoryActions.tasksRejected.add,
                    taskCategoryActions.tasksCancelled.add,
                ...
                if (existingCheck.taskGroup)
                    action.data[parent.parentID] = {
                        ...existingCheck.taskGroup,
                        ...action.data[parent.parentID],
                    };
            }
        }
    ...
    switch (action.type) {
        case taskCategoryActions.tasksNew.put:
            return {
                tasks: { ...filteredTasks, tasksNew: action.data },
                error: null,
        ...
        case taskCategoryActions.tasksNew.add:
            return {
                tasks: {
                    ...filteredTasks,
                    tasksNew: { ...filteredTasks.tasksNew, ...action.data },
                },
                error: null,
                fetched: state.fetched,
            };
        case taskCategoryActions.tasksActive.put:
            return {
                tasks: { ...filteredTasks, tasksActive: action.data },
                error: null,
                fetched: state.fetched,
            };
        ...
        case taskCategoryActions.tasksPickedUp.put:
            return {
                tasks: { ...filteredTasks, tasksPickedUp: action.data },
                error: null,
                fetched: state.fetched,
            };
        ...
        case taskCategoryActions.tasksDelivered.put:
            return {
                tasks: { ...filteredTasks, tasksDelivered: action.data },
                error: null,
                fetched: state.fetched,
            };
        ...
        case taskCategoryActions.tasksRejected.put:
            return {
                tasks: { ...filteredTasks, tasksRejected: action.data },
                error: null,
                fetched: state.fetched,
            };
        ...
        case taskCategoryActions.tasksCancelled.put:
            return {
                tasks: { ...filteredTasks, tasksCancelled: action.data },
                error: null,
                fetched: state.fetched,
            };
