...
function* postNewTask(action) {
    try {
        ...
        const autoAssign =
            action.data.autoAssign.role === "all"
                ? { ...action.data.autoAssign, role: "coordinator" }
                : action.data.autoAssign;
        const result = yield call(
            [api, api.tasks.createTask],
            action.data.payload,
            autoAssign
        );
...
function* postNewTaskRelay(action) {
    try {
        ...
        const previousTask = yield findExistingTask(
            currentTasks,
        ...
        const orderInRelay = result.order_in_relay
            ? parseInt(result.order_in_relay)
            : 0;
        const task = {
            ...emptyTask,
            ...prevTaskData,
            author_uuid: whoami.id,
            uuid: result.uuid,
            order_in_relay: orderInRelay,
            reference: result.reference,
        };
        yield put(
            taskActions.addTaskRelaySuccess({
                payload: task,
                autoAssign: autoAssign,
            })
        );
...
function* updateTask(action) {
    try {
        const api = yield select(getApiControl);
        const result = yield call(
            [api, api.tasks.updateTask],
            action.data.taskUUID,
            action.data.payload
        );
        const data = {
            payload: { ...action.data.payload, etag: result.etag },
...
function* updateTaskPatchFromServer(action) {
    try {
        const api = yield select(getApiControl);
        const result = yield call(
            [api, api.tasks.getTaskAssignedRiders],
            action.data.taskUUID
        );
        const lastResult = result.slice(-1)[0];
        const payload = lastResult
            ? { patch: lastResult.patch, patch_id: lastResult.patch_id }
            : {
                  patch: "",
                  patch_id: null,
              };
        yield put(
            taskActions.updateTaskPatchRequest(action.data.taskUUID, payload)
        );
    } catch (error) {
        yield put(taskActions.updateTaskPatchFailure(error));
    }
...
export function* watchUpdateTaskRequesterContact() {
    yield takeEvery(
        taskActions.updateTaskRequesterContactActions.request,
        updateTaskRequesterContact
    );
}

export function* watchUpdateTaskPickupTime() {
    yield takeEvery(
        taskActions.updateTaskPickupTimeActions.request,
...
export function* watchUpdateTaskDropoffTime() {
    yield takeEvery(
        taskActions.updateTaskDropoffTimeActions.request,
...
export function* watchUpdateTaskTimeOfCall() {
    yield takeEvery(
        taskActions.updateTaskTimeOfCallActions.request,
        updateTaskTimeOfCall
    );
}

export function* watchUpdateTaskPriority() {
    yield takeEvery(
        taskActions.updateTaskPriorityActions.request,
        updateTaskPriority
    );
}
...
export function* watchUpdateTaskPatch() {
    yield takeEvery(
        taskActions.updateTaskPatchActions.request,
        updateTaskPatch
    );
}
...
export function* watchUpdateTaskTimeCancelled() {
    yield takeEvery(
        taskActions.updateTaskCancelledTimeActions.request,
        updateTaskTimeCancelled
    );
}

export function* watchUpdateTaskRejectedTime() {
    yield takeEvery(
        taskActions.updateTaskRejectedTimeActions.request,
        updateTaskRejectedTime
    );
}
...
function* getTasks(action) {
    try {
        ...
        const [
            ...
            call(
                [api, api.tasks.getTasks],
                action.data.userUUID,
                0,
                action.data.role,
                "new",
                0,
...
export function* refreshTasksFromSocket(action) {
    while (true) {
        ...
        const isPostingNewTaskSelector = yield createPostingSelector([
            "UPDATE_TASK_PRIORITY",
        ]);
        const isPosting = yield select((state) =>
...
export function* watchRefreshTasksFromSocket() {
    yield takeLatest(
        taskActions.START_REFRESH_TASKS_LOOP_FROM_SOCKET,
        refreshTasksFromSocket
    );
}
...
function* setRoleViewAndGetTasks(action) {
    ...
    yield put(
        taskActions.getAllTasksRequest(
            action.data.userUUID,
            action.data.page,
            action.data.role
        )
...
export function* watchSetRoleViewAndGetTasks() {
    yield takeLatest(
        taskActions.SET_ROLE_VIEW_AND_GET_TASKS,
