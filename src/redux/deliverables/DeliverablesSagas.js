...
import {availableDeliverablesSelector, deliverablesSelector, getApiControl} from "../Selectors";
...
export function* deleteDeliverable(action) {
    try {
        ...
        yield put(actions.deleteDeliverableSuccess(action.data.deliverableUUID))
        yield put(actions.setDeliverablesSorted())
    } catch (error) {
...
export function* watchDeleteDeliverable() {
    yield takeEvery(actions.deleteDeliverableActions.request, deleteDeliverable)
}
...
export function* watchUpdateDeliverable() {
    yield takeEvery(actions.updateDeliverableActions.request, updateDeliverable)
}
...
export function* watchUpdateDeliverableCount() {
    yield debounce(300, actions.updateDeliverableCountActions.request, updateDeliverableCount);
}
...
export function* watchGetAvailableDeliverables() {
    yield takeLatest(actions.getAvailableDeliverablesActions.request, getAvailableDeliverables)
}

export function* setDeliverablesSorted(action) {
