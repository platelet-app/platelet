...
const getPossibleRiderResponsibilities = async (
  ...
  do {
    ...
    if (body?.data?.getUser?.possibleRiderResponsibilities) {
      items.push(...body.data.getUser.possibleRiderResponsibilities.items);
      nextToken = body.data.getUser.possibleRiderResponsibilities.nextToken;
    } else {
      nextToken = null;
    }
  } while (nextToken);
...
export const handler = async (event: LambdaEvent): Promise<LambdaReturn> => {
  ...
  await pAll(
    filterDeleted.map(
      (r) => () =>
        deletePossibleRiderResponsibilitiesFunction(r, graphQLEndpoint)
    ),
    { concurrency: 10 }
  );
